...
def update_petite_diagonale(inputs):
    ...
    global _dernier_plafond_min_mm
    import math
...
def add_meuble_fields(inputs, cur_mm_func):
    ...
    for field_id, key, default_mm, min_mm, max_mm, label in FIELDS_CAISSON[3:4]:
        add_value_field(gd, field_id, label, mm_to_cm(cur_mm_func(key, default_mm)), min_mm, max_mm)
    _txt_diag_face = gd.addTextBoxCommandInput(
        'textDiagonaleFace', 'Diagonale de la face', '', 1, True)
    _txt_diag_face.isVisible = False
...
class ApercuHTMLEventHandler(adsk.core.HTMLEventHandler):
    ...
    bouton Exporter SVG). window.print() et le telechargement Blob
    classique ne fonctionnent pas dans le navigateur integre des
    palettes Fusion (constate) -- ce canal officiel JS->Python est
    le seul moyen fiable de recuperer un fichier genere cote JS."""
        ...
        try:
            ...
            if args.input.id == 'dropdownMeuble':
                ...
                if not pal:
                    ...
                    _h_apercu = ApercuHTMLEventHandler()
                    pal.incomingFromHTML.add(_h_apercu)
                    apercu_handlers.append(_h_apercu)
